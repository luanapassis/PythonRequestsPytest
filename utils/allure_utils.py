...
class AllureUtils:
    ...
    @classmethod
    def allure_log_requests(cls, url, request_server, query_parameters, json_body, method, headers, cookies):
    ...
    @classmethod
    def allure_log_responses(cls, response_status_code, response_headers, response_text, response):
        allure.attach(str(response_status_code), name="Response StatusCode",
                      attachment_type=allure.attachment_type.TEXT)
        # allure.attach(str(response_headers), name="Response Headers", attachment_type=allure.attachment_type.TEXT)
    ...
    def allure_log_soap_requests(cls, url, xml_body, method, headers):
        allure.attach(str(url), name="URL", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(xml_body), name="Json Body", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(method), name="Method", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(headers), name="Headers", attachment_type=allure.attachment_type.TEXT)
